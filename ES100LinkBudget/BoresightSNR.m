...
%% Power received by one RF emission from REASON upper and lower bound side lobe
    ...
    for i = 1:length
        for j = 1:leneff
            P_r_l_e(i,j) = P_t_l*G_0_l*(lambda_u)^2/(4*pi)*A_eff(i)*N_elm*(1/(4*pi*R^2))*L_pol*eff(j);
        end
    end
...
title("Worst Case Boresight vs. Atenna efficiency 3000km and 1 hours",'FontSize',font)
...
%%W SNR observation time, lower bound efficiency, width
   ...
   T_obs_a_len = width(Obs_a_s);
                ...
                for i = 1:length
                P_r_l_e4(i) = P_t_l*G_0_l*(lambda_l)^2/(4*pi)*A_eff(i)*N_elm*(1/(4*pi*R^2))*L_pol;
                end

                for i = 1:length
                    for j = 1:T_obs_a_len
                        SNR1(i,j) = 10*log((2*P_r_l_e4(i)*Obs_a_s(j)*L_c*(1/(k*T_u))));
                    end
                end
figure()
 plot(Obs_a,SNR1,'LineWidth', linewidth)
 title("Worst Case Boresight SNR vs observation time at 3000km",'FontSize',font)
...
R_a_len = width(R_a);
                ...
                for i = 1:length
                    for j = 1:R_a_len
                        SNR5(i,j) = 10*log((2*P_r_l_e(i,j)*Obs*L_c*(1/(k*T_u))));
                    end
                end
...
title("Worst Case Boresight SNR vs distance signal 1 hour ",'FontSize',font)
...
for j = 1:length
 ...
 for i = 1:length
    for j = 1:w_theta
        P_r_l_e2(i,j) = P_t_l*G_0_l*(lambda_l)^2/(4*pi)*A_eff2(i,j)*N_elm*(1/(4*pi*R^2))*L_pol;
    end
 end
...
title("Worst Case Boresight SNR based on Pointing Direction",'FontSize',font)
...
P_r_l_e3 = zeros(length,w_theta);
    ...
    for i = 1:w_theta
        P_r_l_e3(i) = P_t_l*G_0_l*(lambda_l)^2/(4*pi)*A_eff3(i)*N_elm*(1/(4*pi*R^2))*L_pol;
    end
